...
extern uint32_t nowtime,offtime;
...
@near @interrupt void EXTI_PORTB_IRQHandler (void)
{
  ...
  */
  ...
  {
    IT_Delay(10);                                //延时消抖
    if(GPIO_ReadInputPin(GPIOB,GPIO_PIN_5))                        //确认按下
    {
	...
	{
		IT_Delay(10);                                //延时消抖
    if(GPIO_ReadInputPin(GPIOB,GPIO_PIN_5)==RESET)                        //确认按下
    {
...
@near @interrupt void TIM4_UPD_OVF_IRQHandler (void)
{
  ...
  */
	...
	TIM4_ClearITPendingBit(TIM4_IT_UPDATE);        //清除中断
